...
class ProjectManager extends AbstractManager {
  ...
  findSome(tags) {
    if (tags.length > 0) {
      ...
      const sql = `select p.id, p.title, p.description, p.creationDate, p.client, uc.firstname as creatorFirstname, uc.lastname as creatorLastname, uc.agency as creatorAgency, uc.position as creatorPosition, c.name, c.img, pl.step, group_concat(s.name), group_concat(u.id) from ${this.table} AS p 
